...
class ValueIterationAgent(ValueEstimationAgent):
  ...
  def getQValue(self, state, action):
    ...
    for tmpState in self.mdp.getTransitionStatesAndProbs(state, action)[0]:
    	for prob in self.mdp.getTransitionStatesAndProbs(state, action)[1]:
	   	  newVals = self.values(tmpState)
	      rewards = self.mdp.getReward(state, action, tmpState)
	      qNew + = prob * (rewards + self.discount * newVals)
    return qNew
